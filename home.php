...
<div class="l-cards">
	<div class="p-cards">
		<div class="p-cards__inner l-inner">
				...
				<?php if (have_posts()) : ?>
					<?php while (have_posts()) : the_post(); ?>
						...
						<div class="p-cards__card">
							<div class="p-card p-cardInfo">
								<a href="<?php echo get_permalink(); ?>">
									<figure class="p-card__img p-card__imgInfo">
									...
									<div class="p-card__date p-card__dateInfo">
			...
			<!-- ============= ページング ============= -->
			<?php
			global $wp_query;/* グローバル変数使う時はglobalひつようなのかな？ */
			$big = 1000;
			...
			<?php
			$args = [ /* ①paginate_linksの引数指定 */
				'base' => str_replace($big, '%#%', esc_url(get_pagenum_link($big))), //base %_%に含まれるものをformatから置き換える */
				'format' => '?page=%#%',
				'total' => $wp_query->max_num_pages, // ⑥totalは	⑤にある<pre><?php print_r($wp_query->max_num_pages); ?</pre>8:00~あたり を参考にブラウザから取る。これでマックスの数値が出る
				'current' => $paged, // ④$pagedがなかったら１を表示する */
				'show_all' => FALSE,
				'end_size' => 1,
				'mid_size' => 2,
				'prev_next' => FAlSE,
			...
			$navs = paginate_links($args); //② argsの中で指定した引数を持ったpaginate_linksのnavsという変数 */
			?>
			<?php //③↓$navsがあった場合のみ表示するよう条件分岐する	totalとcurrentが入って表示できるようになる --> -->
			if ($navs) :
				//<pre>?php print_r($navs); ?</pre> ⑥foreach で回して$navsを確認してみると数値が出るのでecho $navにすると数値が反映れる
			?>
